...
\section{Celeriac Output}
This section will give a high level overview of the output of Celeriac.
...
A complete Daikon trace file consists of two parts. The first, called the declaration component, is metadata describing the schema of each program point (method entrance or exit). While Celeriac is inserting instrumentation calls the IL rewriter also makes calls to the \texttt{DeclarationPrinter} class which writes the declaration portion of the trace file.
...
\\ \\
There are some situations meeting where this goal isn't possible. For example, when Celeriac creates an instrumented library that will be used by other programs then the instrumented library must be saved to disk. Also, when Celeriac is run on GUI programs using Windows Presentation Foundation (WPF), Celeriac must be run in off-line mode. This is because WPF programs load resource loading processes cannot complete when the program is in online mode, since the subject program is loaded into the Celeraic launcher's \texttt{AppDomain}, and will attempt to resolve resources from the Celeriac launcher.
...
\end{center} \captionof{figure}{Depiction of the IL rewriter component of Celeriac}
...
\end{center} \captionof{figure}{Depiction of the runtime library component of Celeriac}
\~ \\
...
\begin{itemize}
  \item
 One is the inspect the source code as text, and manually parse the program to learn each expression's type.
  \item
 Another approach is to interoperate with the compiler, since it must already determine each expression's type, and to use this result.
  \item Alternately, variable types may be extracted from a binary program using a bytecode or metadata library.
  \item Finally, the types could be determined by reflection on the binary program, in a structured search to match field names to a list of fields extracted via reflection, which would contain their declared types.
\end{itemize} 
...
\  \\
However, this choice was not implemented. First, it would lead to a much greater increase in code size. Whereas Celeriac inserts 1 new method for each relevant variable (parameter, field, etc.) the proposed alternative would insert 1 new method for each new variable's fields, resulting in much larger instrumented programs. It would require many more context-switches between the client's program and Celeriac's runtime library. Also, much more work being would be done in IL rather than managed C\#, and C\# is much easier to develop in.
