...
A Daikon front-end needs to print out each variable's field consistently for every program point. This is a requirement of Daikon, and exists because Daikon output invariants that are always true. If a variable were present at some program points but not others then Daikon would not be able to reason about that variable. 
\\ \\
...
\\ \\
There are several possible approaches for identifying the declared type of each expression. One is the inspect the source code as text, and manually parse the program to learn each expression's type. Another is to interoperate with the compiler, since it must already determine each expression's type, and to use this result. Alternately, variable types may be extracted from a binary program using a bytecode or metadata library. Finally, the types could be determined by reflection on the binary program.
\\ \\
...
\\ \\
The complication with this approach is to build the appropriate assembly-qualified name given a CCIType. This especially problematic for generic types that may have multiple bounds, e.g. \texttt{public sortEdges<TEdge>(List<TEdge> edges) where TEdge : IEdge<TVertex>, IComparable<TEdge>, IPrettyPrintable}. Generics are reified at runtime in .NET, so at runtime a type \texttt{TEdge} that implements all necessary interfaces would be created, but this type isn't available at instrumentation time, when it is needed by Celeriac. The class \texttt{CeleriacTypeDeclaration} compensates for this, and can be constructed so that a declared types is actually of a list of types, which are a list of constraints on a generic expression. The implementation of this is in \texttt{TypeManager.cs}, and is very complex. Running Celeriac on the Mono test suite helps improve confidence in the results.
\\ \\
Given the complication of this process it's worthwhile to reflect on why this implementation choice was made. Source code parsing alternatives would not be feasible because they fail the design goals of allowing Celeriac to run on multiple languages and being build processes independent (since they require source code). Compiler interop would also fail the build process independence goal, and would be infeasible as well because most .NET code uses Microsoft's proprietary compiler. 
...
\\ \\
An imaginable alternative would insert calls to print the field values directly, and perform
...
\}
\end{Verbatim}
\end{center} \captionof{figure}{Implemented Approach}
\begin{center}
...
\end{Verbatim}
\end{center} \captionof{figure}{Possible Alternative}
\  \\
However, this choice was not implemented for a number of reasons. First, it would lead to a much greater increase in code size. Whereas Celeriac inserts 1 new method for each relevant variable (parameter, field, etc.) the proposed alternative would insert 1 new method for each new variable's fields, resulting in much larger instrumented programs. Also, it would require many more context-switches between the client's program and Celeriac's runtime library. This alternate approach would also lead to much more work being done in IL rather than managed C\#, which is much easier to develop in.
