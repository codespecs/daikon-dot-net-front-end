namespace GeoPoint
{
    public class GeoPointTest
    {
        ...
        public void testEquals()
        {
            ...
            assertTrue("instance not equal to copy.",
        ...
        public void testDistanceTo()
        {
            ...
            assertEquals("From North to Downtown equal to Downtown to North", gpDowntown.distanceTo(gpNorth), gpNorth.distanceTo(gpDowntown), tolerance);
            assertEquals("From North to North 0 miles", 0.0, gpNorth.distanceTo(gpNorth), tolerance);
        }

        public void testHeadingTo()
        {
            assertEquals("Downtown headingTo East should be 90", 90.0, gpDowntown.headingTo(gpEast), tolerance);
            assertEquals("Downtown headingTo West should be 270", 270.0, gpDowntown.headingTo(gpWest), tolerance);

            //assertEquals("Downtown headingTo North should be 0",0.0, gpDowntown.headingTo(gpNorth), tolerance);
            double h = gpDowntown.headingTo(gpNorth);
            assertTrue("Downtown headingTo North should be 0",
                       ((h - 0.0) <= tolerance) || ((360.0 - h) <= tolerance));

            assertEquals("Downtown headingTo South should be 180", 180.0, gpDowntown.headingTo(gpSouth), tolerance);
        ...
        {
            GeoPointTest t = new GeoPointTest("test");
            foreach (var test in t.GetType().GetMethods())
            {
        ...
        private void assertEquals(string msg, double a, double b, double tol)
        {
            if (System.Math.Abs(a - b) > tol && false)
            {
                assertTrue(msg, false);
            }
        }

        private void assertTrue(bool test)
        {
            assertTrue("", test);
        }

        private void assertTrue(string message, bool test)
        {
            if (!test)
            {
